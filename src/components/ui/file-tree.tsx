...
const getFileIcon = (extension?: string) => {
  const iconMap: Record<string, { color: string; icon: string }> = {
    ...
    mp4: { color: "text-[oklch(0.65_0.18_25)]", icon: "▶" },
    mov: { color: "text-[oklch(0.62_0.16_25)]", icon: "▶" },
...
function FileItem({ node, depth, isLast, parentPath }: FileItemProps) {
  ...
  const handleRowClick = () => {
    // Navigate if href — but only if it's a file, or a folder with explicit href
    // (arrow handles folder expand/collapse separately)
    if (node.href) {
...
export function FileTree({ data, className, showHeader = true }: FileTreeProps) {
  return (
    <div
      className={cn(
        showHeader && "bg-file-tree-bg rounded-lg border border-border/50 p-3",
        "font-mono",
        className,
      )}
    >
